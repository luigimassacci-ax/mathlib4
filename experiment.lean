...
theorem iteratedDeriv_bump_one (n : ℕ) : iteratedDeriv n bump 1 = 0 := by
  have h_cont : Continuous (iteratedDeriv n bump) :=
    smooth_bumpAux.continuous_iteratedDeriv n (by exact_mod_cast le_top)
  ...
  have lower_bound : ∀ n : ℕ, g (1 / 2) * 2 ^ (n + 1) ≤
    ...
    have hkey : g (1 / 2) * 2 ^ (n + 1) ≤ |iteratedDeriv (n + 1) g c| / (n + 1).factorial := by
      ...
      have heq : g (1 / 2) * (n + 1).factorial =
          ...
          (n + 1).factorial := by rw [← hrem]
      rw [div_mul_cancel₀ _ hfact_pos.ne'] at heq
      rw [show g (1 / 2) * 2 ^ (n + 1) * (n + 1).factorial =
          |g (1 / 2) * (n + 1).factorial| * 2 ^ (n + 1) by grind,
          ← mul_one |iteratedDeriv (n + 1) g c|, heq, abs_mul, mul_assoc]
      gcongr
      ...
      simp
    calc g (1 / 2) * 2 ^ (n + 1)
        ≤ |(iteratedDeriv (n + 1) (bump : ℝ → ℂ) c).re| / (n + 1).factorial := by
          grw [← iteratedDeriv_re_of_bump, hkey]
      _ ≤ ‖(((derivCLM ℂ ℂ) ^ (n + 1)) bump : ℝ → ℂ) c‖ / (n + 1).factorial := by
          grw [derivCLM_pow_eq]
          exact div_le_div_of_nonneg_right (Complex.abs_re_le_norm _) (hfact_pos.le)
      _ ≤ (⨆ x : ℝ, ‖(((derivCLM ℂ ℂ) ^ (n + 1)) bump : ℝ → ℂ) x‖) / (n + 1).factorial :=
          div_le_div_of_nonneg_right (le_ciSup (isBounded_iteratedDeriv_bump (n+1)) c) hfact_pos.le
...
example (hExp : Summable (fun n : ℕ => ((n.factorial : ℂ)⁻¹ • ((SchwartzMap.derivCLM ℂ ℂ) ^ n))))
  ...
  have h1 : Tendsto (fun n : ℕ => ((n.factorial : ℂ)⁻¹ • (SchwartzMap.derivCLM ℂ ℂ) ^ n) bump)
    ...
    (ContinuousLinearMap.instContinuousEvalConst.continuous_eval_const _ |>.tendsto 0).comp
      (Summable.tendsto_cofinite_zero hExp)
  ...
  simp [-one_div] at *
  set f : 𝓢(ℝ, ℂ) := ((M.factorial : ℂ)⁻¹ • (⇑(derivCLM ℂ ℂ))^[M] bump) with f_def
